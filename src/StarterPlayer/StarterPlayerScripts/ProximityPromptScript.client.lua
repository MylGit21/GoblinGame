...
local function onLoad()
	ProximityPromptService.PromptShown:Connect(function(prompt, inputType)
		...
		local highlightAdornee = prompt:FindFirstAncestorWhichIsA("BasePart") or prompt:FindFirstAncestorWhichIsA("Model")
		local attAdornee = prompt:FindFirstAncestorWhichIsA("BasePart") or prompt:FindFirstAncestorWhichIsA("Model")
		local Active = true
		...
		local function SetNewBeam()
			...
			newBeam.Attachment0 = LocalPlayer.Character.HumanoidRootPart.BodyFrontAttachment
			newBeam.Attachment1 = newatt
			newBeam.Parent = LocalPlayer.Character
		end
		
		local function NewAttachment()
			newatt.Parent = attAdornee
		end
		
		if Active and not highlightAdornee:FindFirstAncestorWhichIsA("Seat") then
			SetHighlight()
			if attAdornee and attAdornee.Transparency < 1 then
				SetNewBeam()
				NewAttachment()
			end
		end
